...
namespace Metrics.Visualization
{
    ...
    {
        private static readonly ILogger log = new LoggerFactory().CreateLogger<MetricsHttpListener>();
            ...
            return Task.Factory.StartNew(async () =>
            {
                ...
                do
                {
                    try
                    {
                        ...
                        {
                            log.LogInformation("HttpListener started successfully after {0} retries", maxRetries - remainingRetries);
                        }
                        remainingRetries = 0;
                    }
                    catch (Exception x)
                    {
                        ...
                        {
                            log.LogWarning("Unable to start HTTP Listener. Sleeping for {0} sec and retrying {1} more times", x, maxRetries - remainingRetries, remainingRetries);
